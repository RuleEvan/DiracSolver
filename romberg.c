...
double Romberg5Vars(double (*f)(double, double, int, double, int), double a, double b, double p, int l, double q, int r_pow, double tol) {
  // Numerical integrator
  int maxiter = 50;
  int maxj = 5;
  double h, g0, fourj, gmax, error, g1, romb;
  ...
  int i, j, jmax, k;
  h = 0.5 * (b - a);
  gmax = h * ((*f)(a, p, l, q, r_pow) + (*f)(b, p, l, q, r_pow));
  g[0] = gmax;
  nint = 1;
  error = 1.0 * pow(10, 20);
  i = 0;
